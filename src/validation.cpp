...
#if defined(NDEBUG)
# error "Happy cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Happy Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s HPPY additional fees, %d delta bytes\n",

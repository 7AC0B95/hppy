Happy Core version *0.15.1* is now available from:

  <https://download.happy.org/happy-0.15.1/>
...
A number of changes to the way Happy Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 glee per kB (0.00001 HPPY). This is to prepare for dropping the minimum transaction fee to 0.1 glee per kB (0.0001 HPPY) in the next release.
